...
module coffee_machine(
    ...
    always @(*) begin
        ...
        case(r_current_state)
            IDLE: begin
                if(w_coin_pulse) begin
                    r_next_state = COIN_IN;
                end
            end
            COIN_IN: begin
                if(return_coin_btn) begin
                    r_next_state = COIN_OUT;
                end else if(coin_val >= COFFEE_VAL) begin
                    r_next_state = READY;
                end
            end
            READY: begin
                if(return_coin_btn || coin_val < COFFEE_VAL) begin
                    r_next_state = COIN_OUT;
                end else if(coffee_btn) begin
                    r_next_state = COFFEE;
                end
            end
            COFFEE: begin
                if(coffee_out) begin
                    r_next_state = READY;   // 커피 제조 완료시 READY로 복귀
                end
            end
            COIN_OUT: begin
                if(coin_val == 0) begin
                    r_next_state = IDLE;    // 잔액 모두 반환시 IDLE
                end
            end
            default: begin
                r_next_state = IDLE;
